...
public class PersonaDAO {
  ...
  public List<UcsawsPersona> obtenerPersonaByIdEvento(Integer idEvento) {
    ...
    query.setTipoQueryGenerico(116);
  ...
  public JSONArray buscarPersona(String codigo) throws ParseException,
    ...
    query
        .setQueryGenerico("select id_persona, per.nombre, per.apellido, fecha_nacimiento, ori.nombre as PaisOrigen, act.nombre as PaisActual, gen.descripcion, ci, tel_linea_baja, tel_celular"
            ...
            + "where upper(per.nombre) like upper('%"
            + codigo
            + "%') or upper(per.apellido) like upper('%"
            + codigo
            + "%')  or"
  ...
  public Boolean eliminarPersona(String codigo) {
    ...
    try {
      ...
      eliminado = false;
      JOptionPane.showMessageDialog(null, "Error al intentar eliminar la Persona.", "Error",
