...
public class NumberToLetterConverter {
  ...
  private static final String[] DECENAS = {"VEINTI", "TREINTA ", "CUARENTA ", "CINCUENTA ",
  ...
  /**
   * Evita la creación una nueva instancia de la clase usando la palabra reservada <code>new</code>.
   */
  ...
  /**
   * Convierte un numero en representacion numerica a uno en representacion de texto. El numero es
   * valido si esta entre 0 y 999'999.999
   * 
  ...
  public static String convertNumberToLetter(double doubleNumber) throws NumberFormatException {
    ...
    int millon =
        Integer.parseInt(String.valueOf(getDigitAt(splitNumber[0], 8))
    ...
    int miles =
        Integer.parseInt(String.valueOf(getDigitAt(splitNumber[0], 5))
    ...
    int cientos =
        Integer.parseInt(String.valueOf(getDigitAt(splitNumber[0], 2))
   ...
   /*5 String valor = splitNumber[1];
    ...
    return converted.toString().replace('*', ' ').trim();
  }
  ...
  static String convertNumber(String number) {
    ...
    int k =
        Integer.parseInt(String.valueOf(getDigitAt(number, 1))
            + String.valueOf(getDigitAt(number, 0)));

    if (k <= 20)
      output.append(UNIDADES[k]);
    else if (k > 30 && getDigitAt(number, 0) != 0)
      output.append(DECENAS[getDigitAt(number, 1) - 2]).append("Y ")
