...
//				row = row + Math.round(dTable[m][n]) +", ";
	...
	int chairs =80;
	double []votes1997 =  new double[]  {2,1,3,400};
	String[] party1997 = {"UUP","DUP","SDLP", "SF","Alliance", "IndU", "UUUC", "WC", "PUP"};
	double []votes =  new double[]  {2,1,3};
//	double[] votes =  new double[]{12.9, 29.3, 13.9, 26.3, 7.7, 2.4, 0.9, 6.6};
	String[] party = {"4 Partido de informaticos",
	    "6 movimiento de programadores","1 ventanas unidas","2 coño"};
...
//Output Results
	for (int p=0; p < votes.length; p++) {
		System.out.println(party[p] + " obtuvo " + allocated [p] + " Escaños.");
	}
