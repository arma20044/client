...
public class VentanaBuscar  extends JFrame implements ActionListener {
	...
	public VentanaBuscar() {
		...
		labelTitulo.setText("BUSQUEDA, MODIFICACION Y ELIMINACION DE GENEROS");
...
//			        	textUsu.setText(selectedData.get(4));
		...
		lblListaDeGeneros.setBounds(325, 156, 213, 30);
...
//
				...
				generoDAO.modificarGenero(txtDesc.getText(), codTemporal);
		...
		if (e.getSource()==botonBuscar)
		{
			...
			if(!(txtCod.getText().length() == 0)  || !(txtDesc.getText().length() == 0)){
					...
					try {
						miPersona = generoDAO.buscarGenero(txtCod.getText(), txtDesc.getText());
...
//			
		...
		if (e.getSource()==botonEliminar)
		{
			if (!codTemporal.equals(""))
			{
				int respuesta = JOptionPane.showConfirmDialog(this,
						"¿Esta seguro de eliminar el Genero " + txtDesc.getText() + "?", "Confirmación",
	...
	private void recuperarDatos(){
		...
		boolean existe=false;
			...
			query.setQueryGenerico("SELECT id_genero, codigo,descripcion, to_char(fch_ins, 'DD/MM/YYYY HH24:MI:SS'),  usuario_ins, to_char(fch_upd, 'DD/MM/YYYY HH24:MI:SS'),usuario_upd   from ucsaws_genero order by descripcion");
