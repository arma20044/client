...
public class CandidatoValidator {
    ...
    public Boolean ValidarCodigo(String codigo, String persona, String lista, String observacion,String idEvento)
	...
	List<UcsawsCandidatos> candidatos = candidatoDAO
		.obtenerCandidatosByIdEvento(Integer.parseInt(idEvento));
	...
	while (ite.hasNext()) {
	    ...
	    if (	(aux.getCodigo().compareToIgnoreCase(codigo) == 0
		    ||	aux.getIdPersona().getIdPersona() == Integer.parseInt(persona)
		    ||	(aux.getIdLista().getIdLista() == Integer.parseInt(lista) &&	aux.getDescripcion().compareTo(observacion) == 0))
